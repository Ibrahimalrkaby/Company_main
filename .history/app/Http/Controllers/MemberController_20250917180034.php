...
class MemberController extends Controller
{
    ...
    public function store(StoreMemberRequest $request)
    {
        ...
        // $image->move(public_path('members'), $newImageName);

        // save the image in the database
        $data['image'] = $newImageName;
        Member::create($data);
        return to_route('admin.member.index')->with('success', __('keywords.cerate_successfully'));
    }
    ...
    public function destroy(Member $member)
    {
        Storage::delete("public/members/$member->image");
        $member->delete();
        return redirect()->route('admin.member.index')->with('success', __('keywords.deleted_successfully'));
    }
}
